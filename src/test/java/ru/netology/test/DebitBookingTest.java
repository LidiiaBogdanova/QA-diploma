...
public class DebitBookingTest {
    ...
    void shouldDebitBookWithCorrectCard() {
        ...
        var status = DBHelper.checkDebitStatus();
    ...
    @Test
    @Description("Тест проверяет, появляется ли уведомление об ошибке о пустом поле CVV")
    ...
    void shouldNotWriteLettersInCvv() {
        ...
        dashboardPage.debitPurchase();
        DataHelper.CardInfo data = DataHelper.getCorrectCardInfo();
        purchasePage.inputData(data.getCardNumber(), data.getMonth(), data.getYear(), data.getCardHolder(), "52а");
        purchasePage.checkIncorrectCvvNotification();
    }
